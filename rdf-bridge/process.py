...
logger_obj.write_log('Program is started with process id: '+str(pid), 1, False)
...
for n,line in enumerate(fp):
    if n+1 in line_numbers: # or n in [25,29] 
        ...
        output = executer_obj.run(cmd, [], False)

        if output[0] == 0:
            result = output[1]
            logger_obj.write_log('Result fo sub on line '+str(n+1)+' with res: '+result, 1, False)
        ...
        if output[0] == 0:
            result = output[1]
            logger_obj.write_log('Result fo pre on line '+str(n+1)+' with res: '+result, 1, False)
            result = result.split(':', 1)
            if len(result) == 2:
                data_ids.append(result[0])
        ...
        if output[0] == 0:
            result = output[1]
            logger_obj.write_log('Result fo obj on line '+str(n+1)+' with res: '+result, 1, False)
            result = result.split(':', 1)
            if len(result) == 2:
                data_ids.append(result[0])
        ...
        if len(data_ids) == 3:
            ...
            logger_obj.write_log('Data length on line '+str(n+1)+' is not 3: '+str(len(data_ids)), 3, False)
